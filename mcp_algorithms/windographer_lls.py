...
"""Windographer 4.2.25-compatible Ordinary Least Squares (LLS) support.

Empirical comparison against the supplied 79117 EL160 4.2.25 export shows
that Q4×S16 local LLS subdivisions with fewer than 50 concurrent time steps
fall back directly to the all-data/global LLS model.

Only LLS mechanics live here.  It does not change the selected target channel
or the calculation and fallback rules of the other MCP methods.
"""
...
LLS_FIT_MIN_SPEED = 2.5
LLS_MIN_LOCAL_SAMPLES = 50  # default 10min; engine passes 24 for verified 1h WG mode  # default 10min; engine passes 24 for verified 1h WG mode
LLS_MIN_DCR = 0.50
...
def build_concurrent_hours(
    ...
    *,
    ...
    minimum_dcr: float = LLS_MIN_DCR,
) -> pd.DataFrame:
    """Create temporary target-local left/left hourly LLS samples.

    DCR applies solely to the temporary MCP training series.  At truncated
    first/last hours, expected 10min points are limited to actual coverage.
    """
    ...
    if target_work.empty:
        raise ValueError("empty LLS target timeline")
...
def _fit_line(frame: pd.DataFrame) -> _LinearModel:
    ...
    if len(work) < 2:
        raise ValueError("not enough points for LLS")
    slope, intercept = np.polyfit(work["ref_ws"].to_numpy(float), work["target_ws"].to_numpy(float), 1)
    return _LinearModel(float(slope), float(intercept))


@dataclass
class WindographerLLSModel:
    global_model: _LinearModel
    local_models: Dict[Tuple[int, int], _LinearModel]
    ...
    def predict_frame(self, frame: pd.DataFrame, *, local_utc_offset_hours: int) -> np.ndarray:
        ...
        if "target_local_time" not in work.columns:
            if "Timestamp_UTC" not in work.columns:
                raise ValueError("LLS prediction needs target_local_time or Timestamp_UTC")
...
def fit_windographer_lls(
    concurrent: pd.DataFrame,
    *,
    fit_min_speed: float = LLS_FIT_MIN_SPEED,
    minimum_local_samples: int = LLS_MIN_LOCAL_SAMPLES,
) -> WindographerLLSModel:
    """Fit OLS globally and for eligible Q4×S16 cells only.

    The 2.5m/s threshold applies when fitting only.  Sparse local cells use
    the global OLS equation directly; there are no quarter-only/sector-only
    ...
    if len(work) < 2:
        raise ValueError("not enough concurrent LLS samples after low-speed filter")
    work["quarter"] = work["target_local_time"].dt.quarter
    work["sector"] = sector_index(work["ref_dir"]).astype(int)
    global_model = _fit_line(work)
    local_models: Dict[Tuple[int, int], _LinearModel] = {}
    for (quarter, sector), group in work.groupby(["quarter", "sector"]):
        if len(group) >= int(minimum_local_samples):
            local_models[(int(quarter), int(sector))] = _fit_line(group)
    return WindographerLLSModel(global_model=global_model, local_models=local_models)
